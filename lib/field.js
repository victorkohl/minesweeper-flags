...
class Field {
   ...
   * @return {Number} The number of flags.
   */
  get numberOfFlags() {
    ...
    allPositions.forEach(position => {
      neighbourhood.forEach(neighbour => {
        position.setNeighbour(
          neighbour,
          this._getNeighbour(neighbour, position)
        );
      });
    });
  }
   ...
   * @return {Promise} A bluebird promise object with the position
   */
  async hitPosition(x, y) {
    if (_.isUndefined(x)) {
      req('x');
    }
    if (_.isUndefined(y)) {
   ...
   * @private
   * @return {array} An array with the position numbers.
   */
  _generateFlags() {
    return _.shuffle(_.range(this.size)).slice(0, this.numberOfFlags);
  }
   ...
   * @param {integer} x - The x coordinate.
   * @param {integer} y - The y coodinate.
   * @return {Boolean} Whether the coordinates are invalid or not.
   */
