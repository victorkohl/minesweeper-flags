  ...
  describe('instance methods', () => {
    ...
    describe('#setNeighbour', () => {
      it('should return a Promise', done => {
        position
          .setNeighbour('nw', new Position(game, 0, 1))
          .should.be.a.Promise(); // eslint-disable-line new-cap
        done();
      });
      ...
      it('does not allow an invalid neighbour', done => {
        position
          .setNeighbour('sse', new Position(game, 0, 1))
          .then(() => should.fail('No error was thrown.'))
          .catch(err => {
            should.exist(err);
            done();
      ...
      it('does not allow an invalid position', done => {
        position
          .setNeighbour('n', 'invalid')
          .then(() => should.fail('No error was thrown.'))
          .catch(err => {
            should.exist(err);
            done();
      ...
      it('increments the "neighbour with flags" counter', done => {
        ...
        position.setNeighbour('ne', new Position(game, 0, 1, true)).then(() => {
    ...
    describe('#hit', () => {
      ...
      it('emits the "position-hit" event telling that a flag was discovered', done => {
        ...
        position.hit().then(() => {
          game.emit
            .calledWith('position-hit', position.x, position.y, true)
            .should.be.true();
          done();
        });
      });
      ...
      it('does not propagate the hits if the position has a flag', done => {
        position.hasFlag = true;
        position.hit().then(() => {
          position.neighbours.forEach(neighbour => {
      ...
      it('emits the "position-hit" event telling the number of nearby flags', done => {
        ...
        position.hit().then(() => {
          game.emit
            .calledWith('position-hit', position.x, position.y, false, 2)
            .should.be.true();
          done();
        });
      });
